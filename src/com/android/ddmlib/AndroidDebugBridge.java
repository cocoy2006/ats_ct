...
public final class AndroidDebugBridge {
   ...
   /**
    ...
    public boolean killAdb() {
    	String os = System.getProperty("os.name");
    	
		String command = "killall -9 -r adb*";
		if (os.startsWith("Windows")) {
			command = "taskkill /F /IM adb*";
		}
		...
		try {  
			...
			if (status != 0) {
	            Log.w(DDMS,
	                    "kill adb failed -- run manually if necessary"); //$NON-NLS-1$
	            return false;
	        }
			
		    Log.d(DDMS, command + " succeeded"); //$NON-NLS-1$
